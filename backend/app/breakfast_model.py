...
model_path = os.path.join(os.path.dirname(__file__), 'breakfast_model.pkl')

# Load the model
with open(model_path, 'rb') as model_file:
    model = pickle.load(model_file)

def predict_breakfast(user_data):
    """
    Predicts breakfast based on the user's data.

    :param user_data: Dictionary containing the user's information.
    :return: Predicted breakfast.
    """
    try:
        ...
        # One-hot encode Diet Type, ensuring consistency with the model
        diet_type = pd.get_dummies([df['Diet Type'][0]], dtype=int)
        expected_diet_types = ['Gluten Free', 'Standard', 'Vegan', 'Vegetarian']
        for dt in expected_diet_types:
            if dt not in diet_type.columns:
                diet_type[dt] = 0
        ...
        model_features = [
            ...
            'Assigned_Breakfast_Calories', 'Waist', 'Diabetes', 'Hypertension',
